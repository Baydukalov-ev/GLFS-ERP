...
Процедура бг_ПередЗаписью(Отказ)
	...
	Если Не ЗначениеЗаполнено(бг_КодНСИ) Тогда
		Максимальныйбг_КодНСИ = бг_ОбщегоНазначенияСервер.МаксимальноеЗначениеСтроковогоРеквизитаСправочника(Ссылка, "Производители", "бг_КодНСИ");
		бг_КодНСИ = бг_ОбщегоНазначенияСервер.ИнкрементированнаяСтрока(
			Максимальныйбг_КодНСИ,
			Метаданные.Справочники.Производители.Реквизиты.бг_КодНСИ.Тип.КвалификаторыСтроки.Длина);
	КонецЕсли;
