...
Функция ЗначениеКонстанты(Идентификатор, Разрез = Неопределено) Экспорт
	...
	Иначе
		Справочники.бг_ДополнительныеКонстанты.НайтиСоздатьКонстанту(Идентификатор);
	КонецЕсли;
...
Функция ЗначенияКонстант(Знач ИдентификаторыСтрокой) Экспорт
	...
	|	ГДЕ
	|		бг_ДополнительныеКонстанты.Идентификатор В(&СписокИдентификаторов)) КАК ЗначенияКонстант
	|
	|СГРУППИРОВАТЬ ПО
	...
	Для каждого СтрокаТаблицыКонстант Из ТаблицаКонстант Цикл
		...
		ОписаниеКонстанты = бг_КонстантыПовтИсп.ОписаниеКонстанты(СтрокаТаблицыКонстант.Идентификатор);
	...
	Для каждого ДанныеЗначенияКонстанты Из ЗначенияКонстант Цикл
		...
		Если ТаблицаКонстант.НайтиСтроки(ПараметрыПоиска).Количество() = 0 Тогда
			Справочники.бг_ДополнительныеКонстанты.НайтиСоздатьКонстанту(ДанныеЗначенияКонстанты.Ключ);
		КонецЕсли;
		
		ОписаниеКонстанты = бг_КонстантыПовтИсп.ОписаниеКонстанты(ДанныеЗначенияКонстанты.Ключ);
...
Процедура УстановитьЗначениеКонстантыБезРазреза(Константа, Значение) Экспорт
	...
	МенеджерЗаписи = РегистрыСведений.бг_ЗначенияДополнительныхКонстант.СоздатьМенеджерЗаписи();
...
Процедура УстановитьЗначенияКонстантыСРазрезом(Константа, Значения) Экспорт
	...
	НаборЗаписей = РегистрыСведений.бг_ЗначенияДополнительныхКонстант.СоздатьНаборЗаписей();
